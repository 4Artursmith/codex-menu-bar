...
struct CLIAccountView: View {
        ...
        Task {
            do {
                try await Task.detached(priority: .userInitiated) {
                    try await CodexCodeSyncService.shared.syncToProfile(profileId)
                }.value
    ...
    private func removeSync() {
        ...
        Task {
            do {
                try await Task.detached(priority: .userInitiated) {
                    try await CodexCodeSyncService.shared.removeFromProfile(profileId)
                }.value
    ...
    private func parseCLIInfo(from json: String) -> CLIAccountInfo? {
        guard let summary = CodexCodeSyncService.shared.extractAccountSummary(from: json) else {
            return nil
        }
        return CLIAccountInfo(email: summary.email, subscriptionType: summary.subscriptionLabel, scopes: summary.scopes)
    }
    ...
    private func extractSessionKey(from json: String) -> String? {
        guard let data = json.data(using: .utf8),
              ...
              let oauth = parsed["codexAiOauth"] as? [String: Any],
