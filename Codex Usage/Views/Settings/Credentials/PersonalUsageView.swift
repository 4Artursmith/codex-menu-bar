...
//  Codex Usage - Codex.ai Personal Usage Tracking
//
...
struct WizardState {
    ...
    var testedOrganizations: [CodexAPIService.AccountInfo] = []
    var selectedOrgId: String? = nil
    var originalSessionKey: String? = nil
    var originalOrgId: String? = nil
}

/// Codex.ai personal usage tracking (free tier)
struct PersonalUsageView: View {
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.section) {
                ...
                HStack(spacing: DesignTokens.Spacing.medium) {
                    Circle()
                        .fill(currentCredentials?.hasCodexAI == true ? Color.green : Color.secondary.opacity(0.4))
                        .frame(width: DesignTokens.StatusDot.standard, height: DesignTokens.StatusDot.standard)

                    VStack(alignment: .leading, spacing: DesignTokens.Spacing.extraSmall) {
                        Text(currentCredentials?.hasCodexAI == true ? "general.connected".localized : "general.not_connected".localized)
                            .font(DesignTokens.Typography.bodyMedium)

                        if let creds = currentCredentials, creds.hasCodexAI {
                            Text(maskKey(creds.codexSessionKey ?? ""))
                    ...
                    if currentCredentials?.hasCodexAI == true {
    ...
    private func loadExistingConfiguration() {
        ...
        if let creds = try? ProfileStore.shared.loadProfileCredentials(profile.id) {
            ...
            wizardState.originalSessionKey = creds.codexSessionKey
        }
    }
    ...
    private func removeCredentials() {
        ...
        do {
            ...
            try profileManager.removeCodexAICredentials(for: profileId)
            ...
            LoggingService.shared.log("PersonalUsageView: Successfully removed Codex.ai credentials")
...
struct EnterKeyStep: View {
    ...
    let apiService: CodexAPIService
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                TextField("sk-codex-sid01-...", text: $wizardState.sessionKey)
            ...
            HStack(spacing: 10) {
                Button(action: {
                    if let url = URL(string: "https://codex.ai") {
                    ...
                    HStack(spacing: 6) {
                        Image(systemName: "safari")
                            .font(.system(size: 12))
                        Text("personal.button_open_codex".localized)
                            .font(.system(size: 12))
                    }
                }
                .buttonStyle(.bordered)
    ...
    private func saveConfiguration() {
        ...
        Task {
            do {
                ...
                creds.codexSessionKey = wizardState.sessionKey
                ...
                if var profile = ProfileManager.shared.activeProfile {
                    profile.codexSessionKey = wizardState.sessionKey
