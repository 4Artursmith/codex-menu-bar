//
//  CodexCodeView.swift
//  Codex Usage - Codex Code Statusline Integration
//
...
/// Codex Code statusline integration settings
struct CodexCodeView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.section) {
                // Page Header
                SettingsPageHeader(
                    title: "codexcode.title".localized,
                    subtitle: "codexcode.subtitle".localized
                )

            // Preview Card (keep as is - user loves it!)
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.medium) {
                HStack {
                    Label("codexcode.preview_label".localized, systemImage: "eye.fill")
                ...
                VStack(alignment: .leading, spacing: DesignTokens.Spacing.small) {
                    ...
                    Text("codexcode.preview_description".localized)
                        .font(DesignTokens.Typography.caption)
                        .foregroundColor(.secondary)
                }
            }
            ...
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.medium) {
                ...
                VStack(alignment: .leading, spacing: DesignTokens.Spacing.small) {
                    Toggle("codexcode.component_directory".localized, isOn: $showDirectory)
                        .font(DesignTokens.Typography.body)

                    Toggle("codexcode.component_branch".localized, isOn: $showBranch)
                        .font(DesignTokens.Typography.body)

                    Toggle("codexcode.component_usage".localized, isOn: $showUsage)
                        .font(DesignTokens.Typography.body)

                    if showUsage {
                        HStack(spacing: 0) {
                            Spacer().frame(width: 20)
                            Toggle("codexcode.component_progressbar".localized, isOn: $showProgressBar)
                                .font(DesignTokens.Typography.caption)
                                .foregroundColor(.secondary)
                        }
                        ...
                        HStack(spacing: 0) {
                            Spacer().frame(width: 20)
                            Toggle("codexcode.component_resettime".localized, isOn: $showResetTime)
                                .font(DesignTokens.Typography.caption)
                                .foregroundColor(.secondary)
                        }
                    }
            ...
            HStack(spacing: DesignTokens.Spacing.iconText) {
                Button(action: applyConfiguration) {
                    Text("codexcode.button_apply".localized)
                        .font(DesignTokens.Typography.body)
                        .frame(minWidth: 70)
                }
                ...
                Button(action: resetConfiguration) {
                    Text("codexcode.button_reset".localized)
                        .font(DesignTokens.Typography.body)
                        .frame(minWidth: 70)
                }
            ...
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.small) {
                ...
                Text("codexcode.requirement_sessionkey".localized)
                    .font(DesignTokens.Typography.caption)
                    .foregroundColor(.secondary)

                Text("codexcode.requirement_restart".localized)
                    .font(DesignTokens.Typography.caption)
                    .foregroundColor(.secondary)
            }
    ...
    /// Applies the current configuration to Codex Code statusline.
    ...
    private func applyConfiguration() {
        ...
        guard showDirectory || showBranch || showUsage else {
            statusMessage = "codexcode.error_no_components".localized
            isSuccess = false
            return
        }
        ...
        guard StatuslineService.shared.hasValidSessionKey() else {
            statusMessage = "codexcode.error_no_sessionkey".localized
            isSuccess = false
            return
        }
        ...
        do {
            ...
            // Update Codex CLI settings.json
            try StatuslineService.shared.updateCodexCodeSettings(enabled: true)

            statusMessage = "codexcode.success_applied".localized
            isSuccess = true
        } catch {
    ...
    /// Disables the statusline by removing it from Codex CLI settings.json.
    private func resetConfiguration() {
        do {
            try StatuslineService.shared.updateCodexCodeSettings(enabled: false)
            statusMessage = "codexcode.success_disabled".localized
            isSuccess = true
        } catch {
            statusMessage = "error.generic".localized(with: error.localizedDescription)
            isSuccess = false
    ...
    private func generatePreview() -> String {
        ...
        return parts.isEmpty ? "codexcode.preview_no_components".localized : parts.joined(separator: " │ ")
    }
}
