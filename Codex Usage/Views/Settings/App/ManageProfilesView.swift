...
struct ManageProfilesView: View {
    ...
    @State private var isAddingCurrentCodexAccount = false
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: DesignTokens.Spacing.section) {
                ...
                SettingsButton(
                    title: "Add Current Codex Account",
                    icon: "person.crop.circle.badge.plus"
                ) {
                    addCurrentCodexAccount()
                }
                .disabled(isAddingCurrentCodexAccount)

                if isAddingCurrentCodexAccount {
                    HStack(spacing: 8) {
                        ProgressView()
                            .scaleEffect(0.8)
                        Text("Adding account from current `codex login`...")
                            .font(.system(size: 11))
    ...
    private func addCurrentCodexAccount() {
        isAddingCurrentCodexAccount = true
        successMessage = nil
        errorMessage = nil
        ...
        Task {
            do {
                ...
                await MainActor.run {
                    successMessage = result.wasCreated
                        ? "Account added. Limits will refresh for this profile."
                        : (switchFlowStarted
                            ? "This account already exists. Authorization opened for another account."
                            : "This account already exists. Please run 'codex logout && codex login' and then add again.")
                    errorMessage = (!result.wasCreated && !switchFlowStarted)
                        ? "Couldn't open authorization automatically. Please run 'codex logout && codex login' in Terminal."
                        : nil
                    isAddingCurrentCodexAccount = false
                }
            } catch {
                if requiresCodexLogin(error) {
                    ...
                    await MainActor.run {
                        successMessage = nil
                        errorMessage = loginStarted
                            ? "Authorization started in Terminal. Complete browser login, then click Add Current Codex Account again."
                            : "Couldn't start login automatically. Run 'codex login' in Terminal, then retry."
                        isAddingCurrentCodexAccount = false
                    }
                    return
                }
...
struct ProfileRow: View {
    ...
    private var profileInfo: String {
        ...
        if let subscription = accountSummary?.subscriptionLabel {
    ...
    private var accountSummary: CodexCLIAccountSummary? {
    ...
    private var profileDisplayName: String {
        if let email = accountSummary?.email, !email.isEmpty {
            return email
        }
        return profile.name
    }
