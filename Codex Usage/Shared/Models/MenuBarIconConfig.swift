...
struct MenuBarIconConfiguration: Codable, Equatable {
    ...
    private static func sanitizeMetrics(_ source: [MetricIconConfig]) -> [MetricIconConfig] {
        var filtered = source.filter { $0.metricType != .api }

        if !filtered.contains(where: { $0.metricType == .session }) {
            filtered.append(.sessionDefault)
        }

        if !filtered.contains(where: { $0.metricType == .week }) {
            filtered.append(.weekDefault)
        }

        let orderMap: [MenuBarMetricType: Int] = [
            .session: 0,
            .week: 1
        ]

        filtered = filtered.map { metric in
            var updated = metric
            updated.order = orderMap[metric.metricType] ?? updated.order
            return updated
        }
