...
/// Service for managing Codex Code statusline configuration.
/// This service handles installation, configuration, and management of the statusline feature
/// for Codex Code terminal integration.
class StatuslineService {
    ...
    /// Swift script that fetches Codex usage data from the API.
    /// Installed to ~/.codex/fetch-codex-usage.swift and executed by the bash statusline script.
...
func readSessionKey() -> String? {
    // Session key injected from Keychain by Codex Usage app
...
func readOrganizationId() -> String? {
    // Organization ID injected from settings by Codex Usage app
...
func fetchUsageData(sessionKey: String, orgId: String) async throws -> (utilization: Int, resetsAt: String?) {
    ...
    guard !orgId.contains(".."), !orgId.contains("/") else {
        throw NSError(domain: "CodexAPI", code: 5, userInfo: [NSLocalizedDescriptionKey: "Invalid organization ID"])
    }

    guard let url = URL(string: "https://codex.ai/api/organizations/\\(orgId)/usage") else {
        throw NSError(domain: "CodexAPI", code: 0, userInfo: [NSLocalizedDescriptionKey: "Invalid URL"])
    }
    ...
    guard let httpResponse = response as? HTTPURLResponse,
        ...
        throw NSError(domain: "CodexAPI", code: 3, userInfo: [NSLocalizedDescriptionKey: "Failed to fetch usage"])
    }
    ...
    throw NSError(domain: "CodexAPI", code: 4, userInfo: [NSLocalizedDescriptionKey: "Invalid response format"])
}
...
"""
    ...
    /// Installed to ~/.codex/statusline-command.sh and configured in Codex Code settings.json.
    /// Reads user preferences from ~/.codex/statusline-config.txt and displays selected components.
    private let bashScript = """
#!/bin/bash
config_file="$HOME/.codex/statusline-config.txt"
...
if [ "$show_usage" = "1" ]; then
  swift_result=$(swift "$HOME/.codex/fetch-codex-usage.swift" 2>/dev/null)
...
"""
    ...
    func installScripts(injectSessionKey: Bool = false) throws {
        let codexDir = Constants.CodexPaths.codexDirectory
        ...
        let swiftDestination = codexDir.appendingPathComponent("fetch-codex-usage.swift")
        ...
        if injectSessionKey {
            ...
            guard let sessionKey = activeProfile.codexSessionKey else {
        ...
        // Install bash script
        let bashDestination = codexDir.appendingPathComponent("statusline-command.sh")
    ...
    func removeSessionKeyFromScript() throws {
        let swiftDestination = Constants.CodexPaths.codexDirectory
            .appendingPathComponent("fetch-codex-usage.swift")
    ...
    func updateConfiguration(
        ...
        let configPath = Constants.CodexPaths.codexDirectory
...
"""
    ...
    /// Enables or disables statusline in Codex Code settings.json
    ...
    func updateCodexCodeSettings(enabled: Bool) throws {
        let settingsPath = Constants.CodexPaths.codexDirectory
            .appendingPathComponent("settings.json")

        let homeDir = Constants.CodexPaths.homeDirectory.path
        let commandPath = "\(homeDir)/.codex/statusline-command.sh"
    ...
    var isInstalled: Bool {
        let swiftScript = Constants.CodexPaths.codexDirectory
            .appendingPathComponent("fetch-codex-usage.swift")

        let bashScript = Constants.CodexPaths.codexDirectory
    ...
    func hasValidSessionKey() -> Bool {
        guard let activeProfile = ProfileManager.shared.activeProfile,
              let key = activeProfile.codexSessionKey else {
            return false
        }
