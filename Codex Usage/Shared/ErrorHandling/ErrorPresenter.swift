...
//  Codex Usage - User-Facing Error Presentation
//
...
class ErrorPresenter {
    ...
    func showInfoAlert(message: String, details: String? = nil, in window: NSWindow? = nil) {
        DispatchQueue.main.async {
            NSApp.activate(ignoringOtherApps: true)
...
#Preview("Error Details - Critical") {
    ErrorDetailsView(
        error: AppError(
            ...
            isRecoverable: false,
            recoverySuggestion: "Please obtain a new session key from codex.ai"
        )
    )
}
