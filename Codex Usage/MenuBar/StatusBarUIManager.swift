...
final class StatusBarUIManager {
    ...
    private var singleProfileRenderCache: [MenuBarMetricType: String] = [:]
    private var multiProfileRenderCache: [UUID: String] = [:]
    ...
    func cleanup() {
        ...
        singleProfileRenderCache.removeAll()
        multiProfileRenderCache.removeAll()
    ...
    func updateMultiProfileButtons(profiles: [Profile], config: MultiProfileDisplayConfig) {
        ...
        for profile in profiles where profile.isSelectedForDisplay {
            ...
            let usage = profile.codexUsage ?? CodexUsage.empty
            ...
            let renderKey = [
                "style:\(config.iconStyle.rawValue)",
                "showWeek:\(config.showWeek)",
                "showLabel:\(config.showProfileLabel)",
                "mono:\(config.useSystemColor)",
                "dark:\(menuBarIsDark)",
                "session:\(Int(sessionDisplay * 10))",
                "week:\(Int(weekDisplay * 10))",
                "sessionStatus:\(String(describing: sessionStatus))",
                "weekStatus:\(String(describing: weekStatus))",
                "name:\(profile.name)"
            ].joined(separator: "|")

            if multiProfileRenderCache[profile.id] == renderKey {
                continue
            }
            ...
            multiProfileRenderCache[profile.id] = renderKey
        }
    }
    ...
    func updateAllButtons(
        ...
        if !hasUsageCredentials || config.enabledMetrics.isEmpty {
            ...
            if let statusItem = statusItems[.session],  // We use .session as placeholder key
               let button = statusItem.button {
                ...
                let menuBarIsDark = button.effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
                let renderKey = "default:\(menuBarIsDark)"
                if singleProfileRenderCache[.session] == renderKey {
                    return
                }
                ...
                singleProfileRenderCache[.session] = renderKey
            }
            return
        }
        ...
        for metricConfig in config.enabledMetrics {
            ...
            // Get actual menu bar appearance from the button
            let menuBarIsDark = button.effectiveAppearance.bestMatch(from: [.darkAqua, .aqua]) == .darkAqua
            let renderKey = singleProfileRenderKey(
            ...
            if singleProfileRenderCache[metricConfig.metricType] == renderKey {
                continue
            }
            ...
            singleProfileRenderCache[metricConfig.metricType] = renderKey
        }
    }
    ...
    func updateButton(
        ...
        if singleProfileRenderCache[metricType] == renderKey {
            return
        }
        ...
        // Non-monochrome needs explicit colors for status indicators
        button.image?.isTemplate = config.monochromeMode
        singleProfileRenderCache[metricType] = renderKey
    }
    ...
    private func observeAppearanceChanges() {
        appearanceObserver = NSApp.observe(\.effectiveAppearance) { [weak self] _, _ in
            self?.singleProfileRenderCache.removeAll()
            self?.multiProfileRenderCache.removeAll()
            self?.delegate?.statusBarAppearanceDidChange()
        }
    }

    private func singleProfileRenderKey(
        ...
        let apiUsed = apiUsage?.currentSpendCents ?? -1
        let apiCredits = apiUsage?.prepaidCreditsCents ?? -1
        let sessionResetBucket = Int(usage.sessionResetTime.timeIntervalSince1970 / 60)

        return [
            "metric:\(metricType.rawValue)",
            "style:\(metricConfig.iconStyle.rawValue)",
            "weekMode:\(metricConfig.weekDisplayMode.rawValue)",
            "apiMode:\(metricConfig.apiDisplayMode.rawValue)",
            "nextSession:\(metricConfig.showNextSessionTime)",
            "mono:\(globalConfig.monochromeMode)",
            "showNames:\(globalConfig.showIconNames)",
            "showRemaining:\(globalConfig.showRemainingPercentage)",
            "dark:\(menuBarIsDark)",
            "session:\(Int(usage.sessionPercentage * 10))",
            "week:\(Int(usage.weeklyPercentage * 10))",
            "weekTokens:\(usage.weeklyTokensUsed)",
            "apiUsed:\(apiUsed)",
            "apiCredits:\(apiCredits)",
            "resetBucket:\(sessionResetBucket)"
        ].joined(separator: "|")
    }
}
