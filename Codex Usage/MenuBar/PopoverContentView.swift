...
struct PopoverContentView: View {
    private struct AccountActionFeedback {
        let text: String
        let isError: Bool
    }
    ...
    @State private var isAddingCurrentAccount = false
    @State private var isAuthorizingAnotherAccount = false
    @State private var accountActionFeedback: AccountActionFeedback?
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            // Direct action outside of Menu to avoid lost clicks in nested menu state
            if showAddAccountQuickAction {
                VStack(alignment: .leading, spacing: 6) {
                    Button(action: addCurrentCodexAccountFromPopover) {
                        HStack(spacing: 8) {
                            Image(systemName: "person.crop.circle.badge.plus")
                                .font(.system(size: 12, weight: .semibold))
                            Text(isAddingCurrentAccount ? "Adding account..." : "Add Current Codex Account")
                                .font(.system(size: 12, weight: .semibold))
                            Spacer()
                            if isAddingCurrentAccount {
                                ProgressView()
                                    .scaleEffect(0.7)
                            }
                        }
                    ...
                    .buttonStyle(.plain)
                    .disabled(isAddingCurrentAccount)

                    Button(action: authorizeAnotherAccountFromPopover) {
                        HStack(spacing: 8) {
                            Image(systemName: "arrow.triangle.2.circlepath.circle")
                                .font(.system(size: 12, weight: .semibold))
                            Text(isAuthorizingAnotherAccount ? "Opening authorization..." : "Authorize Another Account")
                                .font(.system(size: 12, weight: .semibold))
                            Spacer()
                            if isAuthorizingAnotherAccount {
                                ProgressView()
                                    .scaleEffect(0.7)
                            }
                        }
                    ...
                    .buttonStyle(.plain)
                    .disabled(isAuthorizingAnotherAccount)

                    if let feedback = accountActionFeedback {
                        Text(feedback.text)
    ...
    private var showAddAccountQuickAction: Bool {
        true
    }

    private func addCurrentCodexAccountFromPopover() {
        guard !isAddingCurrentAccount else { return }
        LoggingService.shared.log("UI: Add Current Codex Account tapped (popover)")
        isAddingCurrentAccount = true
        accountActionFeedback = AccountActionFeedback(text: "Adding account...", isError: false)

        Task {
            do {
                let result = try await profileManager.addCurrentCodexAccountFromSystem()
                profileManager.activateProfileLocally(result.profile.id)
                NotificationCenter.default.post(name: .manualRefreshRequested, object: nil)

                let switchFlowStarted: Bool
                if result.wasCreated {
                    switchFlowStarted = false
                } else {
                    switchFlowStarted = CodexCLILoginService.shared.startSwitchAccountFlowInTerminal()
                }

                await MainActor.run {
                    isAddingCurrentAccount = false
                    accountActionFeedback = AccountActionFeedback(
                        text: result.wasCreated
                            ? "Account added and activated."
                            : (switchFlowStarted
                                ? "This account already exists. Authorization opened for another account."
                                : "This account already exists. Use 'Authorize Another Account' to login to another account."),
                        isError: !result.wasCreated && !switchFlowStarted
                    )
                }
                clearAccountActionFeedbackAfterDelay()

                if !result.wasCreated {
                    ErrorPresenter.shared.showInfoAlert(
                        message: switchFlowStarted ? "Authorize another account" : "Account already added",
                        details: switchFlowStarted
                            ? "Terminal started 'codex logout && codex login'. Complete browser authorization, then press Add Current Codex Account again."
                            : "This Codex account is already in profiles. Run 'codex logout && codex login' and then add again."
                    )
                }
            } catch {
                if requiresCodexLogin(error) {
                    let loginStarted = CodexCLILoginService.shared.startLoginFlowInTerminal()

                    await MainActor.run {
                        isAddingCurrentAccount = false
                        accountActionFeedback = AccountActionFeedback(
                            text: loginStarted
                                ? "Authorization started. Complete login in browser and tap again."
                                : "Could not open login automatically. Run 'codex login' in Terminal.",
                            isError: !loginStarted
                        )
                    }
                    clearAccountActionFeedbackAfterDelay()
                    ErrorPresenter.shared.showInfoAlert(
                        message: loginStarted ? "Authorization required" : "Manual authorization required",
                        details: loginStarted
                            ? "Terminal opened and started 'codex login'. Complete authorization in browser, then press Add Current Codex Account again."
                            : "Please open Terminal, run 'codex login', finish browser authorization, then press Add Current Codex Account again."
                    )
                    return
                }
                ...
                await MainActor.run {
                    isAddingCurrentAccount = false
                    accountActionFeedback = AccountActionFeedback(text: error.localizedDescription, isError: true)
                }
                clearAccountActionFeedbackAfterDelay()
                ErrorPresenter.shared.showAlert(for: AppError.wrap(error))
            }
        }
    }

    private func authorizeAnotherAccountFromPopover() {
        guard !isAuthorizingAnotherAccount else { return }
        isAuthorizingAnotherAccount = true
        let started = CodexCLILoginService.shared.startSwitchAccountFlowInTerminal()
        isAuthorizingAnotherAccount = false

        accountActionFeedback = AccountActionFeedback(
            text: started
                ? "Authorization opened. Login with another account, then tap Add Current Codex Account."
                : "Could not open authorization. Run 'codex logout && codex login' in Terminal.",
            isError: !started
        )
        clearAccountActionFeedbackAfterDelay()
    }

    private func requiresCodexLogin(_ error: Error) -> Bool {
        if case CodexCodeError.noCredentialsFound = error {
            return true
        }
        if case ProfileError.currentCLIAccountExpired = error {
            return true
        }
        return false
    }

    private func clearAccountActionFeedbackAfterDelay() {
        Task {
            try? await Task.sleep(nanoseconds: 4_000_000_000)
            await MainActor.run {
                accountActionFeedback = nil
            }
        }
    }
}
...
struct ProfileSwitcherCompact: View {
    ...
    var body: some View {
        Menu {
            ForEach(profileManager.profiles) { profile in
                let summary = accountSummary(for: profile)
                Button(action: {
                    Task {
                    ...
                    HStack(spacing: 8) {
                        ...
                        VStack(alignment: .leading, spacing: 1) {
                            Text(summary?.email ?? profile.name)
                                .font(.system(size: 12, weight: .medium))
                                .lineLimit(1)

                            Text(summary?.subscriptionLabel ?? (profile.hasCliAccount ? "CLI Synced" : "No Account"))
                        ...
                        HStack(spacing: 4) {
                            ...
                            // Codex.ai badge
                            if profile.codexSessionKey != nil {
    ...
    private func accountSummary(for profile: Profile) -> CodexCLIAccountSummary? {
        guard let json = profile.cliCredentialsJSON else { return nil }
        return CodexCodeSyncService.shared.extractAccountSummary(from: json)
    }

    private var activeSummary: CodexCLIAccountSummary? {
        guard let profile = profileManager.activeProfile else { return nil }
        return accountSummary(for: profile)
    }

    private var activePrimaryText: String {
        if let email = activeSummary?.email, !email.isEmpty {
            return email
        }
    ...
    private var activeSecondaryText: String {
        if let summary = activeSummary {
            return summary.subscriptionLabel
        }
        if profileManager.activeProfile?.hasCliAccount == true {
            return "CLI Synced"
        }
                ...
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 4) {
                        ...
                        HStack(spacing: 3) {
                            ...
                            if profileManager.activeProfile?.codexSessionKey != nil {
                                Circle()
                                    .fill(Color.blue)
                                    .frame(width: 5, height: 5)
                            }
        ...
        HStack(spacing: 12) {
            ...
            HStack(spacing: 8) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    .buttonStyle(.plain)
                    .help("Click to open status.openai.com")
                }
            }
...
struct SmartUsageDashboard: View {
    ...
    // Always show remaining usage in dashboard cards (e.g., 71% left)
    private var showRemainingPercentage: Bool { true }
    ...
    var body: some View {
        VStack(spacing: isPrimary ? 12 : 8) {
            ...
            VStack(spacing: 6) {
                ...
                HStack {
                    Text(showRemaining ? "\(Int(displayPercentage))% left" : "Used \(Int(displayPercentage))%")
        ...
        VStack(spacing: 12) {
            ...
            HStack {
                VStack(alignment: .leading, spacing: 2) {
                    ...
                    Text("menubar.openai_console".localized)
                        .font(.system(size: 10, weight: .medium))
                        .foregroundColor(.secondary)
                }
