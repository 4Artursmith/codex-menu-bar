...
final class MenuBarIconRenderer {
    ...
    private func createBatteryStyle(
        ...
        if showNextSessionTime && metricType == .session, let resetTime = metricData.sessionResetTime {
            if showIconName {
                // Show "C (→2H)" when labels enabled
                text = "C (\(resetTime.timeRemainingHoursString()))" as NSString
            } else {
            ...
            // Show compact labels
            text = (metricType == .session ? "C" : "W") as NSString
        } else {
    ...
    private func createProgressBarStyle(
        ...
        // For progress bar: show "C" or "W" before the bar (not full prefix)
        ...
        // Draw label before bar (just "C" or "W")
        if showIconName {
            let labelAttributes: [NSAttributedString.Key: Any] = [
                .font: NSFont.systemFont(ofSize: 10, weight: .semibold),
                .foregroundColor: textColor.withAlphaComponent(0.9)
            ]
            let label = (metricType == .session ? "C" : "W") as NSString
    ...
    private func createIconWithBarStyle(
        ...
        // Draw C/W in the CENTER of the circle
        if showIconName {
            let labelAttributes: [NSAttributedString.Key: Any] = [
