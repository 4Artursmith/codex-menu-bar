...
class MenuBarManager: NSObject, ObservableObject {
    ...
    @Published private(set) var usage: CodexUsage = .empty
    @Published private(set) var status: CodexStatus = .unknown
    ...
    private let minimumStatusRefreshInterval: TimeInterval = 60
    ...
    private let statusService = CodexStatusService()
    ...
    // Observer for manual refresh requests (from settings/UI)
    ...
    func setup() {
        ...
        if let profile = profileManager.activeProfile {
            if profile.hasUsageCredentials {
                ...
                if let savedUsage = profile.codexUsage {
                    usage = savedUsage
                }
        ...
        // Pre-warm CLI validity cache off main thread to avoid first-click jank.
        CodexCodeSyncService.shared.prewarmSystemCLIOAuthValidityCache()
        ...
        // Observe manual refresh requests
        observeManualRefreshRequests()
    }
    ...
    func cleanup() {
        ...
        if let manualRefreshObserver = manualRefreshObserver {
            NotificationCenter.default.removeObserver(manualRefreshObserver)
            self.manualRefreshObserver = nil
        }
    ...
    private func observeProfileChanges() {
        ...
        // Observe refresh interval changes on the active profile (same profile ID, different settings)
        profileManager.$activeProfile
            .compactMap { $0?.refreshInterval }
            .removeDuplicates()
            .dropFirst()
            .receive(on: DispatchQueue.main)
            .sink { [weak self] newInterval in
                self?.restartAutoRefreshWithInterval(newInterval)
            }
            .store(in: &cancellables)
    ...
    private func restartAutoRefreshWithInterval(_ interval: TimeInterval) {
        ...
        guard interval > 0 else {
            LoggingService.shared.log("Auto-refresh disabled")
            return
        }
    ...
    private func updateStatusButton(_ button: NSStatusBarButton, usage: CodexUsage) {
    ...
    private func startAutoRefresh() {
        ...
        guard interval > 0 else {
            LoggingService.shared.log("Auto-refresh disabled for active profile")
            return
        }
    ...
    private func observeManualRefreshRequests() {
    ...
    private func refreshAllSelectedProfiles() {
        ...
        let selectedProfiles = profileManager.profiles.filter { $0.isSelectedForDisplay && $0.hasStoredUsageCredentials }
        ...
        refreshTask = Task { [weak self] in
            ...
            for profile in selectedProfiles {
                ...
                do {
                    ...
                    await MainActor.run {
                        self.profileManager.saveCodexUsage(newUsage, for: profile.id)
            ...
            await self.finishRefreshCycle(triggerFollowupIfNeeded: true, showSuccessNotification: false)
        }
    }

    /// Fetches usage data for a specific profile using its credentials
    private func fetchUsageForProfile(_ profile: Profile) async throws -> CodexUsage {
        // Legacy/manual path (session key + org)
        ...
        // CLI OAuth path (no manual keys required)
        return try await apiService.fetchUsageData(for: profile)
    }
    ...
    func refreshUsage() {
        ...
        // Check for usage credentials (Codex.ai or API Console, not just CLI)
        ...
        refreshTask = Task { [weak self] in
            ...
            do {
                let newUsage = try await self.apiService.fetchUsageData()
                ...
                await MainActor.run {
                    ...
                    if let profileId = self.profileManager.activeProfile?.id {
                        self.profileManager.saveCodexUsage(newUsage, for: profileId)
                    }
            ...
            // API credits tracking is intentionally disabled.
            await MainActor.run {
                self.apiUsage = nil
            }

            let shouldNotify = usageSuccess && abs(self.lastRefreshTriggerTime.timeIntervalSinceNow) < 5
            await self.finishRefreshCycle(triggerFollowupIfNeeded: true, showSuccessNotification: shouldNotify)
        }
    }

    private func shouldFetchStatusNow() -> Bool {
        Date().timeIntervalSince(lastStatusFetchAt) >= minimumStatusRefreshInterval || status == .unknown
    }

    @MainActor
    private func finishRefreshCycle(triggerFollowupIfNeeded: Bool, showSuccessNotification: Bool) async {
        refreshTask = nil
        isRefreshing = false
        ...
        guard triggerFollowupIfNeeded else { return }
    ...
    @objc private func preferencesClicked() {
        ...
        // Pause auto-refresh while settings are open to keep UI responsive.
