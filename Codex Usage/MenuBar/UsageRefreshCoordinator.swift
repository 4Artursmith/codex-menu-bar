...
final class UsageRefreshCoordinator {
    ...
    init(
        apiService: APIServiceProtocol = CodexAPIService(),
        statusService: CodexStatusService = CodexStatusService(),
...
protocol UsageRefreshCoordinatorDelegate: AnyObject {
    func usageRefreshDidComplete(usage: CodexUsage)
    func statusRefreshDidComplete(status: CodexStatus)
